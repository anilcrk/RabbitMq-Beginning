...
namespace UdemyRabbitMq.WaterMark.WebUI.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name,Price,Stock,ImageName")] Product product, IFormFile imgFile)
        {
            if (!ModelState.IsValid) return View(product);

            if (imgFile.Length > 0 )
            {
                var randomImageName = Guid.NewGuid() + Path.GetExtension(imgFile.FileName);
